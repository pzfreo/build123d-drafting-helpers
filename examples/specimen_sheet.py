...
SPECS = [
    ...
    ("DatumFeature", 'dt = DatumFeature("A",\n    draft)', "dt", (150, 102)),
    (
        "FeatureControlFrame",
        'fcf = FeatureControlFrame(\n    "position", 0.5,\n    ("A","B","C"), draft,\n    diameter=True, modifier="M")',
        "fcf",
        (-150, 30),
    ),
    ("SurfaceFinish", 'm = SurfaceFinish(\n    "1.6", (0,0,0),\n    draft=draft)', "m", (-50, 30)),
    (
        "DatumTarget",
        'dt2 = DatumTarget(\n    "A1", area_label="Ø6",\n    draft=draft)',
        "dt2",
...
def place_code(snippet, cx, top_y):
    ...
    placed = [
        t.moved(Location((cx - maxw / 2.0, top_y - j * LINE_DY, 0))) for j, t in enumerate(texts)
    ]
    code.extend(placed)
...
tb = TitleBlock(
    ...
    date="2026-06-01",
    width=TB_W,
    draft=draft_preset(font_size=2.6, decimal_precision=1),
)
...
tb_lab = Leader((TB_X0 + 44, tb_top, 0), (TB_X0 + 46, tb_top + 15, 0), "TitleBlock", LBL_DRAFT)
ink.append(tb_lab)
...
for k, ln in enumerate(
    ...
    t = Text(ln, font_size=2.3, font=SANS, align=(Align.MIN, Align.MAX)).moved(
        Location((TB_X0 + 92, tb_top + 18 - k * 3.2, 0))
    )
    fill.append(t)
    note.append(t)
...
def write_specimen_sheet(svg_path: str = "specimen_sheet.svg") -> str:
    ...
    exp.add_layer(
        "stroke", line_color=Color(0, 0, 0), line_weight=0.2, line_type=LineType.CONTINUOUS
    )
    exp.add_layer(
        "ink",
    ...
    exp.add_layer(
        "code", line_color=BLUE, fill_color=BLUE, line_weight=0.0, line_type=LineType.CONTINUOUS
    )
    for bucket, layer in [
        (border, "border"),
        (stroke, "stroke"),
...
if __name__ == "__main__":
    ...
    print(
        f"lint: {sum(i.severity == 'error' for i in issues)} error(s), "

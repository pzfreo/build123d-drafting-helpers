...
def make_drawing(
    ...
    """
    stem = step_file.rsplit(".", 1)[0].rsplit("/", 1)[-1]
    out = out or stem
    for _ext in (".svg", ".dxf"):
    ...
    # Outer diameter (largest Z-axis cylinder, or smallest of x/y for prismatic parts)
    od = a.z_diams[0] if a.z_diams else min(a.x_size, a.y_size)
    _ann(
        Dimension(
...
def _write_script(a) -> str:
    ...
    lint_export = (
        ...
        'open(_stem + ".svg", "w").write(_svgdata)\n'
    ...
    header = (
        ...
        f'"""\n'
        f"{a.title} — Technical drawing ({a.number}).\n"
        f"\n"
